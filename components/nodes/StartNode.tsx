...
const StartNode: React.FC<NodeProps<NodeData>> = ({ data }) => {
  ...
  return (
    <BaseNode data={data} icon={<PlayIcon />}>
      ...
      {config.outputData && (
        <div className="mt-2">
            <p className="font-semibold text-neutral-400">Initial Data:</p>
            <pre className="text-xs p-2 rounded-md bg-neutral-700/50 font-mono whitespace-pre-wrap break-all max-h-24 overflow-y-auto">
                {config.outputData}
            </pre>
        </div>
      )}
